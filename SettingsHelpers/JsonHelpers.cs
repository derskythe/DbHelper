...
internal static class JsonHelpers {
  ...
  /// <exception cref="ArgumentException">Path cannot be null or empty -
  /// path</exception>
  public static void ReplaceNested(this JObject self, string path,
                                   JToken value) {
    if (self is null) {
  ...
  /// <returns>System.ValueTuple&lt;System.Boolean,
  /// List&lt;DetectedChanges&gt;&gt;.</returns>
  public static (bool IsEqual, List<DetectedChanges> Differences)
      CompareJson(this JToken sourceToken, JToken targetToken,
    ...
    if (sourceToken == null) {
      return (false, new List<DetectedChanges> { new DetectedChanges(
                         MissedSide.Source, DifferenceType.Null) });
    }

    if (targetToken == null) {
      return (false, new List<DetectedChanges> { new DetectedChanges(
                         MissedSide.Target, DifferenceType.Null) });
    }
    ...
    case JTokenType.Object: {
      ...
      // var differentValues = model.Properties().Select(c =>
      // c.Path).Except(current.Properties().Select(c => c.Path));
      var unchangedKeys =
          current.Properties()
      ...
      foreach (var token in calculated) {
        ...
        differences.Add(
            new DetectedChanges(MissedSide.Target, sourceToken[token]));
      }

      foreach (var token in removedKeys) {
        differences.Add(
            new DetectedChanges(MissedSide.Source, targetToken[token]));
      }
    ...
    case JTokenType.Array: {
      ...
      var plus =
          new JArray(current.Except(model, new JTokenEqualityComparer()));
      var minus =
          new JArray(model.Except(current, new JTokenEqualityComparer()));
    ...
    default:
      ...
      if (sourceHasValue && targetHasValue &&
          sourceValue.IsEqual(targetValue)) {
        return (true, new List<DetectedChanges>(0));
      }

      if (forcedList.Contains(sourceToken.Path)) {
        return (false, new List<DetectedChanges> { new DetectedChanges(
                           DifferenceType.ForcedChange, sourceToken.Path,
                           sourceValue, targetValue) });
      } else {
        return (false, new List<DetectedChanges> { new DetectedChanges(
                           sourceValue, sourceToken.Path, targetValue,
                           sourceHasValue) });
      }
    }
