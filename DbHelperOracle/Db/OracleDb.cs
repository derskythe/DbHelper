...
internal static class OracleDb {
  ...
  public static void Init(string hostName, string user, string pass,
                          string dbase, string port, bool perfCounter = false) {
    _ConnectionString =
    ...
    //$"Data Source={hostName}:1521/XE;Persist Security Info=True;User
    //ID={user};Password={pass}";
    /*_ConnectionString = "User ID=" + user + ";Data Source=" + dbase +
       ";Password=" + pass +
                        ";Min Pool Size=1;Max Pool Size=150;Pooling=True;" +
                        "Validate Connection=true;Connection
       Lifetime=300;Connection Timeout=300";*/
  ...
  private static int? GetInt(OracleDataReader reader, string order) {
    return reader[order] != DBNull.Value ? Convert.ToInt32(reader[order])
                                         : null;
  }

  private static string GetString(OracleDataReader reader, string order) {
    return reader[order] != DBNull.Value ? Convert.ToString(reader[order])
                                         : null;
  }

  private static DateTime? GetDateTime(OracleDataReader reader, string order) {
    return reader[order] != DBNull.Value ? Convert.ToDateTime(reader[order])
                                         : null;
  }
  ...
  public static List<ComboboxItem> ListViews() {
    ...
    try {
      ...
      connection.Open();
      const string sql = "SELECT view_name FROM user_views UNION SELECT " +
                         "table_name view_name FROM user_tables";
      ...
      if (reader.HasRows) {
        while (reader.Read()) {
          ...
          result.Add(new ComboboxItem { Id = item, Value = item,
  ...
  public static List<ComboboxItem> ListProcedures(string ownerName) {
    ...
    try {
      ...
      const string sql =
          "SELECT p.OBJECT_NAME,p.PROCEDURE_NAME FROM SYS.ALL_PROCEDURES p " +
          "WHERE p.OBJECT_NAME = p.OBJECT_NAME AND p.OBJECT_TYPE = " +
          "'PROCEDURE' AND p.OWNER = :ownerName AND PROCEDURE_NAME IS NULL " +
      ...
      command.BindByName = true;
      command.Parameters
          .Add("ownerName", OracleDbType.Varchar2, ParameterDirection.Input)
  ...
  public static List<ComboboxItem> ListPackages(string ownerName) {
    ...
    try {
      ...
      const string sql =
          "SELECT p.OBJECT_NAME,p.PROCEDURE_NAME FROM SYS.ALL_PROCEDURES p " +
          "WHERE p.OBJECT_NAME = p.OBJECT_NAME AND p.OBJECT_TYPE = 'PACKAGE' " +
          "AND p.OWNER = :ownerName AND PROCEDURE_NAME IS NOT NULL ORDER BY " +
      ...
      if (reader.HasRows) {
        while (reader.Read()) {
          ...
          result.Add(new ComboboxItem { Id = $"{objectName}.{procedureName}",
  ...
  public static ProcedureInfo
  ListProcedureParameters(ComboboxItem desiredProcedure) {
    OracleConnection connection = null;
    var result = new ProcedureInfo(0, desiredProcedure.AdditionalData,
    ...
    try {
      ...
      if (string.IsNullOrEmpty(desiredProcedure.AdditionalData)) {
        const string sql =
            "SELECT t.ARGUMENT_NAME, t.in_out, t.DATA_TYPE FROM " +
            "SYS.ALL_ARGUMENTS t WHERE OBJECT_NAME = :procName AND " +
            "package_name IS NULL ORDER BY t.SEQUENCE";
        ...
        // command.Parameters.Add("packageName", OracleDbType.Varchar2,
        // ParameterDirection.Input).Value = package;
        command.Parameters
        ...
        if (reader.HasRows) {
          ...
          while (reader.Read()) {
            ...
            var info = new ParameterInfo(
                name, reader["DATA_TYPE"].GetString(),
                reader["in_out"].GetString().IsEqual("IN"), i++,
                reader["DATA_TYPE"].GetString().GetNetType(),
                name.ToUpperCamelCase(false), name.ToLowerCamelCase(false));
        ...
        const string sql = "SELECT " +
                           " t.ARGUMENT_NAME, t.in_out, t.DATA_TYPE FROM " +
                           "SYS.ALL_ARGUMENTS t " +
                           " WHERE OBJECT_NAME = :procName AND package_name " +
                           "= :packageName ORDER BY t.SEQUENCE";
  ...
  public static ProcedureInfo ListProcedureParameters(string ownerName,
                                                      string package,
    ...
    try {
      ...
      if (!string.IsNullOrEmpty(package)) {
        const string sql =
            "SELECT t.ARGUMENT_NAME, t.in_out, t.DATA_TYPE FROM " +
            "SYS.ALL_ARGUMENTS t WHERE package_name = :packageName AND " +
            "OBJECT_NAME = :procName AND owner = :owner AND ARGUMENT_NAME IS " +
            "NOT NULL ORDER BY t.SEQUENCE";
        ...
        const string sql =
            "SELECT t.ARGUMENT_NAME, t.in_out, t.DATA_TYPE FROM " +
            "SYS.ALL_ARGUMENTS t WHERE package_name IS NULL AND OBJECT_NAME " +
            "= :procName AND owner = :owner AND ARGUMENT_NAME IS NOT NULL " +
            "ORDER BY t.SEQUENCE";
  ...
  public static List<KeyValuePair<string, string>>
  ListColumns(string tableOrView) {
    OracleConnection connection = null;
    ...
    try {
      connection = new OracleConnection(_ConnectionString);
      connection.Open();
      const string sql = "SELECT t.column_name, t.DATA_TYPE FROM " +
                         "user_tab_cols t WHERE table_name = :viewName";
      ...
      command.BindByName = true;
      command.Parameters
          .Add("viewName", OracleDbType.Varchar2, ParameterDirection.Input)
          .Value = tableOrView;
