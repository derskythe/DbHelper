...
internal struct ParameterInfo
{
    ...
    {
        return $"{nameof(DbName)}: {DbName}, {nameof(DbType)}: {DbType}, {nameof(InParam)}: {InParam}, {nameof(Index)
        }: {Index}, {nameof(NetType)}: {NetType}, {nameof(Name)}: {Name}, {nameof(NameLowerCamelCase)}: {
            NameLowerCamelCase}";
    }
}
