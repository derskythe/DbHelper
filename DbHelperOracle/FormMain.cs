...
public partial class FormMain : Form {
  ...
  private void Connect() {
    ...
    try {
      OracleDb.Init(txtHostname.Text, txtUsername.Text, txtPassword.Text,
                    txtServiceName.Text, txtPort.Text);
  ...
  private void ButtonRefresh_Click(object sender, EventArgs e) { Connect(); }

  private void UpdateTableCombo() {
    ComboTableForProcedureGenerate.Items.Clear();

    // ReSharper disable once CoVariantArrayConversion
    ComboTableForProcedureGenerate.Items.AddRange(
        OracleDb.ListTables().ToArray());
  ...
  private void UpdateProcCombo() {
    ...
    ComboProcedureList.Items.AddRange(
        OracleDb.ListPackages(ownerName).ToArray());
    ComboProcedureList.Items.AddRange(
        OracleDb.ListProcedures(ownerName).ToArray());
  ...
  private void ButtonGenerateProcedure_Click(object sender, EventArgs e) {
    try {
      if (ComboProcedureList.SelectedItem is not ComboboxItem selectedItem ||
          string.IsNullOrEmpty(selectedItem.Id)) {
        return;
      }

      txtProcedure.Text = Utils.GenerateProcedure(
          _Settings.DbConfig.Username.ToUpperInvariant(),
          selectedItem.AdditionalData, selectedItem.ClearName,
  ...
  private void ComboProcedureList_SelectedIndexChanged(object sender,
                                                       EventArgs e) {
    ButtonGenerateProcedure.PerformClick();
  }

  private void
  ComboTableForProcedureGenerate_SelectedValueChanged(object sender,
                                                      EventArgs e) {
