...
internal static partial class Utils {
  ...
  GenerateViewFunction(string className,
                       IReadOnlyList<KeyValuePair<string, string>> list,
                       string selectedItem) {
    var funcData = new StringBuilder();

    if (IS_STATIC) {
      funcData.Append("public static async Task<List<")
          .Append(className)
          .Append(">> List")
          .Append(className)
          .Append("()\r\n{");
  ...
  public static string GenerateProcedure(string ownerName, string packageName,
                                         string procName,
                                         bool radioSeparateChecked) {
    var list =
        OracleDb.ListProcedureParameters(ownerName, packageName, procName);
    ...
    foreach (var paramList in list.ParamList) {
      ...
      foreach (var info in paramList) {
        strParamList.Append("new DbParam(\"")
            .Append(info.DbName)
            .Append("\", ")
            .Append(info.NetType.GetOracleType())
            .Append(", ");
        ...
        if (!info.InParam) {
          ...
          if (radioSeparateChecked) {
            strArgumentsList.Append(info.NetType)
                .Append(' ')
                .Append(info.NameLowerCamelCase)
                .AppendLine(",");
      ...
      if (listOutputTypes.Count > 0) {
        if (listOutputTypes.Count > 1) {
          ...
          str.Append(")> ")
              .Append(upperPackageName)
              .Append(upperProcName)
              .Append("(\r\n");
        } else {
          str.Append("public static async Task<");
          str.Append(string.Join(',', listOutputTypes));
      ...
      if (listOutputTypes.Count > 0) {
        ...
        str.Append(">(\"")
            .Append(string.IsNullOrEmpty(packageName)
                        ? ownerName
  ...
  public static string
  GenerateClassData(string className,
                    IReadOnlyList<KeyValuePair<string, string>> list) {
    var classData = new StringBuilder();

    classData.Append("[Serializable, XmlRoot(\"")
        .Append(className)
        .Append("\")]\r\n");
    ...
    foreach (var pair in list) {
      ...
      classData.Append(pair.Value.GetNetType()).Append(' ').Append(lower);
    ...
    foreach (var pair in upperList) {
      classData.Append(pair).Append(": ").Append('{').Append(i).Append("}, ");
  ...
  private static string GetOracleType(this string type) {
    return type switch { "long" => "OracleDbType.Int64",
