...
public partial class Db {
  ...
  private static DbCommand GetCommand(NpgsqlConnection connection,
                                      string commandText,
  ...
  protected static NpgsqlParameter GetParameter(string parameter, object value,
                                                NpgsqlDbType? sqlType = null) {
    ...
    if (value != null) {
      ...
      if (type.IsEnum) {
        ...
        parameterObject =
            new NpgsqlParameter(parameter, JsonConvert.SerializeObject(value));
      } else {
  ...
  protected static NpgsqlParameter GetParameter(string parameter, object value,
    ...
    if (logValues) {
      Log.Debug(
          $"parameterObject type: {parameterObject.NpgsqlDbType}, Value: {parameterObject.Value}");
    }
  ...
  protected async Task ExecuteNonQuery(
      string procedureName, IReadOnlyList<NpgsqlParameter> parameters,
    ...
    try {
      ...
      Log.Error(
          ex, $"{ex.Message}. Failed to ExecuteNonQuery for {procedureName}, " +
  ...
  ExecuteScalar<T>(string procedureName,
  ...
  protected async Task<int> Count(string tableName,
                                  IReadOnlyList<NpgsqlParameter> parameters) {
    int returnValue;
    NpgsqlConnection connection = null;
    ...
    try {
      ...
      await using var cmd = GetCommand(
          connection, $"SELECT COUNT(*) FROM {tableName}", CommandType.Text);
      ...
      Log.Error(ex, $"{ex.Message}. Failed to Count for {tableName}, " +
                        $"parameters: {parameters?.GetStringFromArray()}");
  ...
  protected async Task<DbDataReader>
  GetDataReader(string procedureName, IReadOnlyList<NpgsqlParameter> parameters,
    ...
    try {
      ...
      {
        await using var cmd =
            GetCommand(connection, procedureName, commandType);
  ...
  protected async Task<DbDataReader>
  GetDataReader(string procedureName,
  ...
  protected async Task<T>
  SelectSingle<T>(string cmdText, IReadOnlyList<NpgsqlParameter> parameters,
  ...
  SelectSingle<T>(string cmdText, IReadOnlyList<NpgsqlParameter> parameters,
    ...
    await using var dataReader =
        await GetDataReader(cmdText, parameters, commandType);
  ...
  Many<T>(string cmdText, IReadOnlyList<NpgsqlParameter> parameters,
          Func<DbDataReader, T> funcName) {
