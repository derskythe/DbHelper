...
internal static class Converter {
  public static int ToInt(DbDataReader row) { return row[0].GetInt(); }
  ...
  public static string GetNetType(this string dbType) {
    ...
    if (dbType.Contains("VARCHAR") || dbType.Contains("CHARACTER VARYING") ||
  ...
  public static ProcedureInfo ToProcedureInfo(DbDataReader row) {
    return new ProcedureInfo { Name = row["routine_name"].GetString(),
                               DbType = row["data_type"].GetString(),
                               NetType =
                                   row["data_type"].GetString().GetNetType(),
                               SpecificName =
                                   row["specific_name"].GetString() };
  }
