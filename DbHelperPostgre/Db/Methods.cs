...
public partial class Db {
  public async Task<bool> CheckConnection() {
    try {
      var result = await SelectSingle("SELECT 1", null, Converter.ToInt);
  ...
  public async Task<List<string>> ListTables() {
    return await Many(
        ...
        "'BASE TABLE' AND table_schema NOT IN('pg_catalog', " +
        "'information_schema') ORDER BY table_name",
        null, Converter.ToStringValue);
  }

  public async Task<List<string>> ListViews() {
    return await Many(
        "SELECT table_name FROM information_schema.tables WHERE table_type = " +
        "'VIEW' AND table_schema NOT IN('pg_catalog', 'information_schema') " +
        "ORDER BY table_name",
        null, Converter.ToStringValue);
  }
  ...
  public async Task<List<ProcedureInfo>> ListProcedures() {
    const string sql =
        "SELECT r.routine_name, r.data_type, r.specific_name FROM " +
        "information_schema.routines r WHERE r.specific_schema NOT " +
        "IN('pg_catalog', 'information_schema') ORDER BY r.routine_name";
  ...
  public async Task<List<ParameterInfo>> ListProcedureParameters(string name) {
    var parameterList =
        new List<NpgsqlParameter> { GetParameter("@procName", name) };
    const string sql =
        @"SELECT r.routine_name, p.parameter_mode, p.parameter_name, p.data_type, p.ordinal_position
...
ORDER BY r.routine_name, p.ordinal_position";
  ...
  public async Task<List<ParameterInfo>> ListColumns(string tableName,
                                                     ObjectType objectType) {
    var parameterList =
        new List<NpgsqlParameter> { GetParameter("@name", tableName) };
    var sql = objectType == ObjectType.Table ? @"select
       c.column_name,
