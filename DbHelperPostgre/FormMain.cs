...
public partial class FormMain : Form {
  ...
  private async Task ConnectDb() {
    try {
      ...
      _DataAccess = new Db.Db(new DbConfigSettingsElement() {
        ...
        Password = txtPassword.Text, Database = txtServiceName.Text, Port = port
      });
  ...
  private async void UpdateViewCombo() {
    ...
    foreach (var item in tables) {
      ComboView.Items.Add(
          new ComboboxItem { Id = item,
                             Value = $"{item} ({objectType.AsString()})",
                             ObjectType = objectType });

      ComboTablesForProcedureGeneration.Items.Add(
          new ComboboxItem { Id = item,
                             Value = $"{item} ({objectType.AsString()})",
                             ObjectType = objectType });
    }
  ...
  private async void UpdateProcCombo() {
    ...
    foreach (var item in list) {
      ComboProcedureList.Items.Add(new ComboboxItem {
        Id = item.SpecificName, Value = $"{item.Name} ({item.DbType})",
        ObjectType = ObjectType.Procedure, AdditionalData = item.DbType,
        ClearName = item.Name
      });
    }
  ...
  private async void FormMain_Load(object sender, EventArgs e) {
    ...
    if (!string.IsNullOrEmpty(txtHostname.Text) &&
        !string.IsNullOrEmpty(txtServiceName.Text) &&
  ...
  private async void ButtonGenerateView_Click(object sender, EventArgs e) {
    try {
      ...
      if (sel.ObjectType == ObjectType.View &&
          sel.Value.StartsWith("v_", StringComparison.OrdinalIgnoreCase)) {
        className = className[1..];
      }

      // Gen class
      txtClass.Text =
          FormatUsingRoslyn(Utils.GenerateClassData(className, list));
      ...
      var htmlViewFunction =
          formatter.GetHtmlString(txtViewFunction.Text, Languages.CSharp);
  ...
  private async void ButtonGenerateProcedure_Click(object sender, EventArgs e) {
    try {
      ...
      txtProcedure.Text = FormatUsingRoslyn(Utils.GenerateProcedure(
          sel.ClearName, sel.AdditionalData, paramList, radioSeparate.Checked));
  ...
  private async void ButtonGeneratePlSql_Click(object sender, EventArgs e) {
    try {
      if (ComboTablesForProcedureGeneration.SelectedItem is not ComboboxItem
              comboBoxItem) {
        return;
      }
  ...
  private void
  ComboTablesForProcedureGeneration_SelectedIndexChanged(object sender,
                                                         EventArgs e) {
  ...
  private async
      Task InitializeAsync(Microsoft.Web.WebView2.WinForms.WebView2 component) {
