...
public abstract partial class BaseDataAccess {
  public async Task<List<string>> ListTables() {
    return await Many("SELECT NAME FROM SYS.TABLES", null, CommandType.Text,
                      Converter.ToStringValue);
  }

  public async Task<List<string>> ListViews() {
    return await Many("SELECT NAME FROM SYS.VIEWS", null, CommandType.Text,
                      Converter.ToStringValue);
  }

  public async Task<List<string>> ListProcedures() {
    return await Many("SELECT NAME FROM SYS.PROCEDURES", null, CommandType.Text,
                      Converter.ToStringValue);
  }

  public async Task<List<ParameterInfo>> ListProcedureParameters(string name) {
    var parameterList = new DbParameter[] { GetParameter("@procName", name) };
    ...
    return await Many(sql, parameterList, CommandType.Text,
                      Converter.ToParameterInfo);
  }

  public async Task<List<ParameterInfo>>
  ListProcedureColumns(string procedureName) {
    var parameterList =
        new DbParameter[] { GetParameter("@procedureName", procedureName) };
    const string sql = "sp_describe_first_result_set @procedureName";
    return await Many(sql, parameterList, CommandType.Text,
                      Converter.ToProcedureParameterInfo);
  }

  public async Task<List<ParameterInfo>> ListColumns(string tableName,
                                                     ObjectType objectType) {
    var parameterList = new DbParameter[] { GetParameter("@name", tableName) };
