...
public static class Converter {
  public static int ToInt32(DbDataReader row) { return row[0].GetInt(); }
  ...
  public static ParameterInfo ToParameterInfo(DbDataReader row) {
    return new ParameterInfo(row["order_num"].GetInt() - 1,
                             row["name"].GetString(), row["type"].GetString(),
  ...
  public static ParameterInfo ToProcedureParameterInfo(DbDataReader row) {
    return new ParameterInfo(
        row["column_ordinal"].GetInt() - 1, row["name"].GetString(),
        row["system_type_name"].GetString(),
        row["system_type_name"].GetString().GetNetType(), true);
  }
  ...
  public static string GetNetType(this string msSqlDbType) {
    ...
    if (msSqlDbType.Contains("DECIMAL") || msSqlDbType.Contains("MONEY") ||
        msSqlDbType.Contains("SMALLMONEY")) {
      return "decimal";
    }

    if (msSqlDbType.Contains("VARCHAR") || msSqlDbType.Contains("NVARCHAR") ||
        msSqlDbType.Contains("CHAR") || msSqlDbType.Contains("NCHAR") ||
        msSqlDbType.Contains("NTEXT") || msSqlDbType.Contains("TEXT")) {
      return "string";
    }
    ...
    if (msSqlDbType.Contains("DATE") || msSqlDbType.Contains("DATETIME") ||
