...
public abstract partial class BaseDataAccess {
  ...
  private DbCommand GetCommand(DbConnection connection, string commandText,
                               CommandType commandType) {
    var command = new SqlCommand(
        commandText, connection as SqlConnection) { CommandType = commandType };
  ...
  protected SqlParameter GetParameter(string parameter, SqlDbType dbType,
                                      object value) {
    var parameterObject = new SqlParameter(parameter, value ?? DBNull.Value) {
      Direction = ParameterDirection.Input, SqlDbType = dbType
    };
  ...
  protected SqlParameter GetParameterOut(
      string parameter, SqlDbType type, object value = null,
    ...
    if (type == SqlDbType.NVarChar || type == SqlDbType.VarChar ||
        type == SqlDbType.NText || type == SqlDbType.Text) {
  ...
  ExecuteNonQuery(string procedureName, IReadOnlyList<DbParameter> parameters,
  ...
  protected async Task<T1>
  ExecuteNonQuery<T1>(string procedureName,
    ...
    try {
          ...
          if (parameter.Direction == ParameterDirection.Output ||
            ...
            if (string.IsNullOrEmpty(returnParam1)) {
              ...
              throw new ArgumentException(
                  $"Too many OUTPUT params. Already set {returnParam1}");
            }
          }
      ...
      if (string.IsNullOrEmpty(returnParam1)) {
        throw new ArgumentException(
            $"Didn't find Output or InputOutput params. {nameof(returnParam1)}: {returnParam1}");
      }
  ...
  protected async Task<(T1, T2)>
  ExecuteNonQuery<T1, T2>(string procedureName, List<DbParameter> parameters,
                          CommandType commandType = CommandType.StoredProcedure)
      where T1 : IConvertible
    ...
    try {
        ...
        foreach (var parameter in parameters) {
          if (parameter.Direction == ParameterDirection.Output ||
            ...
            if (string.IsNullOrEmpty(returnParam1)) {
              ...
              throw new ArgumentException(
                  $"Too many OUTPUT params. Already set {returnParam1}, {returnParam2}");
            }
          }
      ...
      if (string.IsNullOrEmpty(returnParam1) ||
          string.IsNullOrEmpty(returnParam2)) {
        throw new ArgumentException(
  ...
  protected async Task<(T1, T2, T3)> ExecuteNonQuery<T1, T2, T3>(
      string procedureName, List<DbParameter> parameters,
      CommandType commandType = CommandType.StoredProcedure)
      where T1 : IConvertible
      where T2 : IConvertible
    ...
    try {
        ...
        foreach (var parameter in parameters) {
          if (parameter.Direction == ParameterDirection.Output ||
            ...
            if (string.IsNullOrEmpty(returnParam1)) {
              ...
              throw new ArgumentException(
                  $"Too many OUTPUT params. Already set {returnParam1}, {returnParam2}, {returnParam3}");
            }
          }
      ...
      if (string.IsNullOrEmpty(returnParam1) ||
        ...
        throw new ArgumentException(
            $"Didn't find Output or InputOutput params. " +
  ...
  protected async Task<(T1, T2, T3, T4, T5, T6)>
  ExecuteNonQuery<T1, T2, T3, T4, T5, T6>(
      string procedureName, List<DbParameter> parameters,
      CommandType commandType = CommandType.StoredProcedure)
      where T1 : IConvertible
      where T2 : IConvertible
    ...
    try {
      ...
      if (string.IsNullOrEmpty(returnParam1) ||
          string.IsNullOrEmpty(returnParam2) ||
          string.IsNullOrEmpty(returnParam3) ||
          string.IsNullOrEmpty(returnParam4) ||
          string.IsNullOrEmpty(returnParam5) ||
          string.IsNullOrEmpty(returnParam6)) {
        throw new ArgumentException(
            $"Didn't find Output or InputOutput params. " +
            $"{nameof(returnParam1)}: {returnParam1}, " +
            $"{nameof(returnParam2)}: {returnParam2}" +
      ...
      return (ExtractValue<T1>(returnValue1), ExtractValue<T2>(returnValue2),
              ExtractValue<T3>(returnValue3), ExtractValue<T4>(returnValue4),
              ExtractValue<T5>(returnValue5), ExtractValue<T6>(returnValue6));
    } catch (Exception ex) {
      Log.Error(
          ex,
  ...
  ExecuteScalar<T>(string procedureName, IReadOnlyList<DbParameter> parameters)
      where T : class {
    T returnValue;
    SqlConnection connection = null;
  ...
  private async Task<DbDataReader>
  GetDataReader(string procedureName, IReadOnlyList<DbParameter> parameters,
                CommandType commandType = CommandType.StoredProcedure) {
    DbDataReader ds;
  ...
  protected async Task<List<T>> Many<T>(string sqlValue,
  ...
  private async Task<List<T>> Many<T>(string sqlValue,
                                      IReadOnlyList<DbParameter> paramList,
                                      CommandType commandType,
                                      Func<DbDataReader, T> funcName) {
    var result = new List<T>();
    ...
    await using var dataReader =
        await GetDataReader(sqlValue, paramList, commandType);
  ...
  protected async Task<T> Single<T>(string sqlValue,
                                    IReadOnlyList<DbParameter> paramList,
                                    Func<DbDataReader, T> funcName) {
    await using var dataReader = await GetDataReader(sqlValue, paramList);
