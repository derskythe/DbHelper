...
internal static class Utils
{
        ...
        string selectedItem)
    {
        ...
        funcData.Append("return await Many(query, paramList, Converter.To").Append(className).Append(");\r\n}\r\n\r\n");
    ...
    {
        var funcData = new StringBuilder();
        var singleRecord = selectedItem.StartsWith("get", StringComparison.Ordinal);
        ...
        foreach (var pair in returnedFields)
        {
            ...
            {
                funcData.Append(char.ToUpperInvariant(pair.NetType[0])).Append(pair.NetType.Substring(1));
            }
    ...
    public static string GenerateClassData(string className, List<ParameterInfo> list)
    {
        ...
        {
            classData.Append(upperList[i]).Append(" = ").Append(lowerList[i]).Append(";\r\n");
            i++;
        }
...
*/
    ...
    public static string GetClassName(this string value)
    {
        ...
        else if (value.StartsWith("list", StringComparison.Ordinal))
        {
    ...
    private static string GetDbParamType(this string msSqlDbType)
    {
        ...
        foreach (var name in Enums.GetNames(typeof(SqlDbType)))
        {
