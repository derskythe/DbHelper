...
namespace DbHelper
{
    internal static class Utils
    {
            ...
            string selectedItem)
        {
            ...
            foreach (var pair in list)
            {
                var paramName = ToUpperCamelCase(pair.Name, true);
        ...
        {
            var funcData = new StringBuilder();
            funcData.Append("public async Task ").Append(ToUpperCamelCase(selectedItem, true));
            ...
            if (radioSeparateChecked)
            {
                ...
                {
                    funcData.Append(info.NetType).Append(' ').Append(ToLowerCamelCase(info.Name, false));
                    i++;
            ...
            if (paramList.Count > 0)
            {
                ...
                foreach (var info in paramList)
                {
                    ...
                    else
                    {
                        ...
                        if (radioSeparateChecked)
                        {
                            funcData.Append(ToLowerCamelCase(info.Name, false));
                        }
                        else
                        {
            ...
            bool radioSeparateChecked)
        {
            ...
            if (singleRecord)
            {
                funcData.Append("public async Task<")
                        .Append(className)
                        .Append("> ")
                        .Append(ToUpperCamelCase(selectedItem, true));
            }
            else
            {
                funcData.Append("public async Task<List<")
        ...
        public static string GenerateClassData(string className, List<ParameterInfo> list)
        {
            ...
            foreach (var pair in list)
            {
                var upper = ToUpperCamelCase(pair.Name, true);
            ...
            foreach (var pair in list)
            {
                var lower = ToLowerCamelCase(pair.Name, true);
